...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/zhoushengliang/v2-ui/master/v2-ui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://raw.githubusercontent.com/zhoushengliang/v2-ui/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
