...
install_v2-ui() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/zhoushengliang/v2-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 v2-ui 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 v2-ui 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 v2-ui 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/v2-ui-linux-${arch}.tar.gz https://github.com/zhoushengliang/v2-ui/releases/download/${last_version}/v2-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/zhoushengliang/v2-ui/releases/download/${last_version}/v2-ui-linux-${arch}.tar.gz"
    ...
    echo -e ""
    curl -o /usr/bin/v2-ui -Ls https://raw.githubusercontent.com/zhoushengliang/v2-ui/master/v2-ui.sh
